use std::{io::{BufRead, BufReader, ErrorKind}, os::unix::net::UnixStream};
...
fn read_event(line: &str) -> Result<Event> {
	let Some((event_name, params)) = line.split_once(">>") else { bail!("Could not find separator in {line}") };
	Ok(match event_name {
		...
		"activespecial" => {
			...
			Event::ActiveSpecial {
				monitor_name: String::from(monitor_name),
				workspace_name: if workspace_name.is_empty() { None } else  { Some(String::from(workspace_name)) },
			}
		},
		"activespecialv2" => {
			params!(params => workspace_id,, workspace_name,, monitor_name);
		...
		"submap" => Event::Submap {
			name: if params.is_empty() { None } else { Some(String::from(params)) },
		},
		"changefloatingmode" => {
...
} impl Iterator for Socket2 {
	type Item = Result<Event>;
	fn next(&mut self) -> Option<Self::Item> {
		let mut buf = String::new();
		match self.stream.read_line(&mut buf) {
			Ok(_) => {
				Some(read_event(buf.trim()))
			},
